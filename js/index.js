import { LISTA_PLANES, LISTA_PORCENTAJES, LISTA_LINEAS, LISTA_PLANES_FAMILIARES } from "../json/lista_planes.js";
    ...
    rd_tipo_cliente() {
        ...
        rd_tipo_cliente.forEach(radio => {
            radio.addEventListener("change", (e) => {
                ...
                if (tipo_cliente === "1") {
                    document.querySelectorAll("#container_calculadora .select-estado").forEach(select => {
    ...
    btn_calcular() {
        document.querySelector("#btn_calcular").addEventListener("click", () => {
            ...
            if (numero_lineas === 0 || costo_plan_familiar === 0) {
                alert("Seleccionar las opciones necesarias!");
                return;
            }

            debugger
            pago_sin_descuento = numero_lineas * costo_plan_familiar;
            pago_con_descuento = ((numero_lineas - 1) * promocion) + costo_plan_familiar;
            pago_ahorro_familiar = pago_sin_descuento - pago_con_descuento;
            pago_total_plan_actual = Math.round((plan_linea_1 - (plan_linea_1 * descuento_linea_1)) + (plan_linea_2 - (plan_linea_2 * descuento_linea_2)) + (plan_linea_3 - (plan_linea_3 * descuento_linea_3)) + (plan_linea_4 - (plan_linea_4 * descuento_linea_4)) + (plan_linea_5 - (plan_linea_5 * descuento_linea_5)), 2);
            pago_diferencia_factura = pago_total_plan_actual === 0 ? 0 : pago_con_descuento - pago_total_plan_actual;

            document.querySelector("#txt_pago_sin_descuento").value = pago_sin_descuento;
            document.querySelector("#txt_pago_con_descuento").value = pago_con_descuento;
            document.querySelector("#txt_pago_ahorro_plan_familiar").value = pago_ahorro_familiar;
            document.querySelector("#txt_pago_total_plan_actual").value = pago_total_plan_actual
            document.querySelector("#txt_pago_diferencia_factura").value = pago_diferencia_factura;
        });
    },
    btn_limpiar() {
        document.querySelector("#btn_limpiar").addEventListener("click", () => {
            document.querySelectorAll("#container_calculadora select").forEach(select => {
                select.selectedIndex = "0";
            });

            document.querySelectorAll("#container_calculadora input[type='text']").forEach(input => {
                input.value = null;
            });
